...
module Sapristi
  RSpec.describe MonitorManager do
    ...
    let(:a_monitor) do
      { id: 0, name: a_monitor_name, main: '*', x: 3840, y: 2160, offset_x: 0, offset_y: 0, work_area: [100, 200, 400, 600], work_area_height: 400, work_area_width: 300 }.transform_keys(&:to_s)
    end
   ...
   1: +#{another_monitor_name} 1920/509x1080/286+#{another_monitor_offset_x}+#{another_monitor_offset_y}  HDMI-1)
    ...
    context('#get_monitor') do
      before(:each) do
        ...
        allow_any_instance_of(LinuxXrandrAdapter).to receive(:get_monitors_work_area).and_return(0 => work_area, 1 => work_area)
      end
