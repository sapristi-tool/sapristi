...
RSpec.describe Sapristi do
  it "has a version number" do
    expect(Sapristi::VERSION).not_to be nil
  end

  it("defaults to configuration file $HOME/.sapristi.csv") do
  	configuration_loader = spy(ConfigurationLoader.new)

  	under_test = Sapristi::Sapristi.new(configuration_loader: configuration_loader)
  ...
  it("creates empty configuration file if it does not exist") do
  	...
  	File.delete mock_conf_file if File.exists? mock_conf_file
  	allow_any_instance_of(under_test.class).to receive(:user_default_configuration_file).and_return(mock_conf_file)

  	under_test.run

  	expect(File.exists? mock_conf_file).to be_truthy
  end

  it 'process lines in configuration file' do
  	definition = { "Title" => nil, "Command" => "sol", "Monitor" => nil, "X-position" => 0, "Y-position" => 0, "H-size" => 500, "V-size" => 500, "Workspace" => nil }
  	...
  	Sapristi::Sapristi.new(definition_processor: definition_processor).run file_path
  ...
  it 'appends line number to error when processing file' do
  	definition = { "Title" => nil, "Command" => "", "Monitor" => nil, "X-position" => 0, "Y-position" => 0, "H-size" => 500, "V-size" => 500, "Workspace" => nil }
  	...
  	expect { Sapristi::Sapristi.new().run file_path }.to raise_error Error
  end
end
